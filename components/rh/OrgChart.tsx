...
const OrgChart: React.FC<OrgChartProps> = ({ onNavigate }) => {
  ...
  const [draggedEmpName, setDraggedEmpName] = useState<string>('');
  ...
  const handleAddLevel = (e: React.FormEvent) => {
    ...
    showAlert('Nouveau niveau ajouté à l\'organigramme');
    loadOrgChartData();
  };
  ...
  const handleDragLeave = (e: React.DragEvent) => {
    // Ignorer les leave sur les enfants du card
  ...
  const handleDrop = (e: React.DragEvent, target: Employee) => {
    ...
    setPendingChange({
      draggedId: draggedEmpId,
      draggedName: draggedEmpName,
      targetId: target.id,
  ...
  const handleConfirmManagerChange = async () => {
    ...
    try {
      ...
      await apiClient.put(`/hr/employees/${pendingChange.draggedId}`, {
        managerId: pendingChange.targetId
      });
  ...
  const filterEmployees = (employees: Employee[]): Employee[] => {
    return employees.filter(emp => {
      ...
      const matchesDepartment = selectedDepartment === 'all' || emp.dept === selectedDepartment;
  ...
  const getDepartments = (): string[] => {
    ...
    if (orgData.type === 'hierarchical' && orgData.orgChart) {
      const collect = (employees: Employee[]) => {
        employees.forEach(emp => {
          if (emp.dept && emp.dept !== 'N/A') departments.add(emp.dept);
          collect(emp.children);
        });
      };
      collect(orgData.orgChart);
  ...
  // ---- Render node with drag & drop ----

  const renderNode = (node: Employee) => {
    const isDragging  = draggedEmpId === node.id;
    const isDropTarget = dropTargetId === node.id && draggedEmpId !== node.id;
    ...
    return (
      <div key={node.id} className="flex flex-col items-center">
        <div
          ...
          title="Glisser-déposer pour changer le manager"
          className={[
            'relative p-6 bg-white rounded-[2rem] border shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group cursor-grab active:cursor-grabbing w-64 text-center select-none',
            isDragging   ? 'opacity-40 scale-95 border-indigo-300' : 'border-slate-100',
            isDropTarget ? 'border-2 border-indigo-500 ring-4 ring-indigo-100 shadow-indigo-200' : ''
          ].join(' ')}
        >
          {isDropTarget && (
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-indigo-500 text-white text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full whitespace-nowrap shadow-lg">
              Nouveau manager
            </div>
          )}
          <div className="w-16 h-16 bg-slate-100 rounded-[1.5rem] overflow-hidden mx-auto mb-4 border-4 border-white shadow-md group-hover:scale-110 transition-transform">
            <img
              ...
              onError={(e) => { (e.target as HTMLImageElement).src = `https://picsum.photos/seed/${node.id}/100/100`; }}
            />
          </div>
  ...
  const renderDepartmentView = (departments: { [key: string]: Employee[] }) => {
    const filtered = Object.entries(departments).reduce((acc, [dept, employees]) => {
      const f = employees.filter(emp => {
        const matchesSearch = searchTerm === '' ||
          emp.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          emp.role.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      if (f.length > 0) acc[dept] = f;
      return acc;
    }, {} as { [key: string]: Employee[] });

    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {Object.entries(filtered).map(([dept, employees]) => (
          <div key={dept} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
            <div className="text-center mb-8">
              <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight mb-2">{dept}</h3>
            ...
            <div className="space-y-6">
              {employees.map(emp => (
                <div
                  ...
                  onDragEnd={handleDragEnd}
                  className={[
                    'flex items-center gap-4 p-4 rounded-2xl transition-all cursor-grab active:cursor-grabbing',
                    dropTargetId === emp.id && draggedEmpId !== emp.id
                ...
                >
                  <div className="w-12 h-12 bg-slate-200 rounded-xl overflow-hidden flex-shrink-0">
                    <img
                      ...
                      className="w-full h-full object-cover"
                      referrerPolicy="no-referrer"
                      onError={(e) => { (e.target as HTMLImageElement).src = `https://picsum.photos/seed/${emp.id}/100/100`; }}
                    />
                  </div>
                  <div className="flex-grow min-w-0">
                    ...
                    {emp.contracts && emp.contracts.length > 0 && (
                      <p className="text-[8px] font-bold text-emerald-600">{emp.contracts[0].type} - Actif</p>
                    )}
                  </div>
                </div>
              ))}
  ...
  const renderOrgContent = () => {
    if (loading) {
      return (
        <div className="flex flex-col items-center justify-center h-full min-h-[400px]">
          ...
          <p className="text-slate-600 font-bold uppercase text-[10px] tracking-widest">Chargement de l'organigramme...</p>
        </div>
      );
    }
      ...
      return (
        <div className="flex flex-col items-center justify-center h-full min-h-[400px]">
          <div className="bg-red-50 border border-red-200 rounded-3xl p-8 text-center max-w-md">
            ...
            <button onClick={loadOrgChartData} className="px-6 py-3 bg-red-600 text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:bg-red-700 transition-all">
              Réessayer
            </button>
          </div>
        </div>
      ...
      return (
        <div className="flex flex-col items-center justify-center h-full min-h-[400px]">
          <div className="bg-slate-100 border border-slate-200 rounded-3xl p-8 text-center max-w-md">
            ...
            <p className="text-slate-500 text-sm">Ajoutez des employés et des contrats pour voir l'organigramme.</p>
          </div>
        </div>
      );
    }
        ...
        <div className="flex justify-center">
          <div className="w-fit">
            {filtered.length > 0
              ...
              : <div className="text-center py-12"><p className="text-slate-500 font-bold">Aucun employé ne correspond aux filtres</p></div>
            }
          </div>
        </div>
      );
    ...
    if (orgData.type === 'flat' && (orgData as any).employees) {
      return renderDepartmentView({ 'Tous les employés': (orgData as any).employees });
    }

    if (orgData.departments) return renderDepartmentView(orgData.departments);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium uppercase text-[10px] tracking-[0.3em]">
              Structure hiérarchique · Glisser-déposer pour réorganiser
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-2xl px-6 py-3 flex items-center gap-3">
        ...
        <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">
          Glissez une carte sur une autre pour changer le manager de l'employé
        </p>
      </div>
      ...
      {/* ---- Modal confirmation changement manager ---- */}
      <HRModal
        isOpen={!!pendingChange}
        onClose={() => setPendingChange(null)}
        title="Confirmer le changement de manager"
        size="sm"
        footer={
          <div className="flex justify-end gap-4">
            ...
            <button
              ...
              className="px-10 py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl disabled:opacity-60"
            >
              {changingManager ? 'Modification...' : 'Confirmer'}
            </button>
          </div>
        }
      >
          ...
          <div className="space-y-6">
            <div className="p-6 bg-indigo-50 rounded-[2rem] border border-indigo-100">
              <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                  <p className="text-sm font-black text-slate-900">Nouveau lien hiérarchique</p>
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest">Cette action modifie le manager direct</p>
                </div>
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Employé déplacé</span>
                <span className="text-sm font-bold text-slate-900">{pendingChange.draggedName}</span>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-indigo-50 rounded-2xl">
                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Nouveau manager</span>
                <span className="text-sm font-bold text-indigo-700">{pendingChange.targetName}</span>
              </div>
            </div>
          </div>
        )}
      ...
      >
        <div className="space-y-6">
          ...
          <form className="space-y-6" onSubmit={handleAddLevel}>
