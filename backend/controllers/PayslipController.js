import { Employee, Contract, Tenant, Department, Advance, Prime } from '../models/index.js';
...
export class PayslipController {
  ...
  static async downloadAllAsZip(req, res) {
    try {
      ...
      for (const employee of employeesWithContracts) {
        ...
        const socialContributions = Math.round(grossSalary * 0.092);
        const incomeTax = Math.max(0, Math.round((grossSalary - 30000) * 0.15));
        const totalDeductions = socialContributions + incomeTax + Math.round(totalAdvanceDeductions);
        ...
        zip.file(`${safeName}_${month}.html`, html);
        generated++;
      }
  ...
  static generateZipPayslipHTML({ employee, contract, tenant, month, empPrimes, empAdvances, calculations }) {
    const { baseSalary, totalPrimes, grossSalary, socialContributions, incomeTax,
            totalAdvanceDeductions, totalDeductions, netSalary, currency } = calculations;
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Cotisations sociales salarié (9,2%)</td>
      <td class="cat deduction">Retenue</td>
